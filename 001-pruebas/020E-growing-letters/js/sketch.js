...
let effectTransparency = 1;
let filamentThickness = 1;
let drawnSegments = [];
...
let effectTransparencySlider, effectTransparencyValue;
let filamentThicknessSlider, filamentThicknessValue;
let modeButtons = {};
...
const MAX_ACTIVE_BRANCHES = 600;
const MAX_DRAWN_SEGMENTS = 9000;
...
class Branch {
  constructor(x, y, angle, parentAngle = null, generation = 0, audioLevel = 0, centerX = letterCenter.x, centerY = letterCenter.y) {
    ...
    this.maxDistance = random(90, 180) + generation * 45;
  ...
  update(attractX = null, attractY = null, audioLevel = 0) {
    ...
    if (drawnSegments.length > MAX_DRAWN_SEGMENTS) {
      drawnSegments.splice(0, drawnSegments.length - MAX_DRAWN_SEGMENTS);
    }
    ...
    let canBranchByImpulse = random() < (0.0003 + audioLevel * 0.005);
    ...
    if (shouldBranch) {
      if (this.children.length < (this.generation < 2 ? 1 : 1) && this.generation < 5) {
        let spreadBase = this.generation === 0 ? PI / 12 : PI / 9;
        let branchSpread = spreadBase * (0.8 + random(0.6, 1.2));
...
function setup() {
  ...
  effectTransparencySlider = document.getElementById('effectTransparencySlider');
  effectTransparencyValue = document.getElementById('effectTransparencyValue');
  filamentThicknessSlider = document.getElementById('filamentThicknessSlider');
  filamentThicknessValue = document.getElementById('filamentThicknessValue');
  ...
  effectTransparencySlider.addEventListener('input', (e) => {
    const sliderValue = parseFloat(e.target.value);
    effectTransparency = sliderValue;
    effectTransparencyValue.textContent = sliderValue.toFixed(2).replace(/\.00$/, '').replace(/0$/, '');
    if (!isGrowing) redrawCanvas();
  });

  filamentThicknessSlider.addEventListener('input', (e) => {
    const sliderValue = parseFloat(e.target.value);
    filamentThickness = sliderValue;
    filamentThicknessValue.textContent = sliderValue.toFixed(2).replace(/\.00$/, '').replace(/0$/, '');
    if (!isGrowing) redrawCanvas();
  });
...
function getCssFont(size = fontSize) {
  const fontSpec = getSelectedFontSpec();
  return `${fontSpec.weight} ${size}px ${fontSpec.family}`;
}

function applyTextFont(targetContext = drawingContext) {
  const cssFont = getCssFont();
...
function measureTextWidth(text, size = fontSize) {
  const ctx = getMeasureContext();
  ctx.font = getCssFont(size);
  return ctx.measureText(text).width;
}

function configureTextContext(ctx, size = fontSize, color = '#ffffff') {
  ctx.font = getCssFont(size);
  ctx.textAlign = 'left';
...
function getEffectAlpha() {
  return Math.round(constrain(effectTransparency, 0, 1) * 255);
}

function getTextColorWithAlpha(alpha = getEffectAlpha()) {
  const hex = String(textColor || '#ffffff').replace('#', '');
...
function getRenderedStrokeWeight(weight) {
  return (weight || 0.8) * filamentThickness;
}
...
async function updateTimestamp() {
  try {
    const response = await fetch('js/sketch.js?v=2026-08-23-1604', { method: 'HEAD' });
...
function getLineWidth(line) {
  ...
  for (let i = 0; i < line.length; i++) {
    const char = line[i];
    widthTotal += measureTextWidth(char);
    if (i < line.length - 1) {
...
function renderTextBlock(target, layout, color = '#ffffff', scale = 1) {
  const ctx = target && target.fillText ? target : null;
  if (!ctx) return;
  configureTextContext(ctx, fontSize * scale, color);
  ctx.textAlign = 'center';

  for (let i = 0; i < layout.lines.length; i++) {
    const block = layout.layout[i];

    for (let char of block.chars) {
      ctx.fillText(char.char, char.x * scale, char.y * scale);
    }
  }
}
...
function updateLetterBoundary() {
  ...
  pg.clear();
  configureTextContext(pg.drawingContext, fontSize, textColor);
  pg.drawingContext.textAlign = 'center';

  for (let i = 0; i < layout.lines.length; i++) {
    const block = layout.layout[i];

    for (let char of block.chars) {
      pg.drawingContext.fillText(char.char, char.x, char.y);
    }
  }
...
function setLetterConfigurationLocked(isLocked) {
  ...
  if (textColorSwatches) textColorSwatches.forEach((button) => button.disabled = isLocked);
...
function stopGrowth() {
  ...
  if (soundFile && typeof soundFile.onended === 'function') {
    try {
      soundFile.onended(() => {});
    } catch (e) {
      console.warn('Sound end callback cleanup failed:', e);
    }
  }
...
function redrawCanvas() {
  ...
  stroke(getTextColorWithAlpha());
  const previewSegments = drawnSegments.slice(-Math.min(6000, drawnSegments.length));
  for (let seg of previewSegments) {
    strokeWeight(getRenderedStrokeWeight(seg.weight));
    line(seg.x1, seg.y1, seg.x2, seg.y2);
  }

  fill(theme.foreground);
  textAlign(CENTER, CENTER);
  renderTextBlock(drawingContext, layout, textColor);
}
...
function exportPNG() {
  ...
  exportGraphics.stroke(getTextColorWithAlpha());
  for (let seg of drawnSegments) {
   exportGraphics.strokeWeight(getRenderedStrokeWeight(seg.weight) * scale);
   exportGraphics.line(seg.x1 * scale, seg.y1 * scale, seg.x2 * scale, seg.y2 * scale);
  }

  exportGraphics.textAlign(CENTER, CENTER);
  renderTextBlock(exportGraphics.drawingContext, layout, textColor, scale);
...
function draw() {
  ...
  if (isGrowing) {
    ...
    // Keep enough live filaments for an organic effect without letting the colony
    // become large enough to choke the render loop.
    branches = branches.filter(b => b.isActive);
    if (branches.length > MAX_ACTIVE_BRANCHES) {
      branches = branches.slice(-MAX_ACTIVE_BRANCHES);
    }
  }
}
